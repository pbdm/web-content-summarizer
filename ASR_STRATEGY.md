...
## 1. 主动默认值选择 (Active Default Selection)

为了防止在 CPU 环境下意外触发大模型下载（3GB+）导致的任务阻塞，系统会根据硬件环境主动设置默认值：

| 硬件环境 | 默认模型 (`--model`) | 默认计算类型 (`--compute-type`) | 默认并行数 |
| :--- | :--- | :--- | :--- |
| **GPU (CUDA)** | `large-v3` | `float16` | 1 |
| **CPU** | `base` | `int8` | 4 |

## 2. 硬件层级自适应 (Hardware Fallback)
